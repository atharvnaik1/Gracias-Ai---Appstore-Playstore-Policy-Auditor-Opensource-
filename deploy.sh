...
# ─── 1. System packages ───────────────────────
echo "==> [1/9] Updating system & installing dependencies..."
apt-get update -qq
...
if [ ! -f "$APP_DIR/.env.local" ]; then
    ...
    echo "==> [5/9] .env.local exists. Skipping."
fi
...
DEPLOY_LOG="/var/log/ipaash_vercel_deploy.log"
{
    ...
    vercel --prod --token "$VERCEL_TOKEN" --team "$VERCEL_TEAM_SLUG" --confirm
    DEPLOY_EXIT=$?
